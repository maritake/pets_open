...
//クリックされた投稿を取得
$sql_post = 
'SELECT m.name, po.member_id, po.pet_id, pe.pet_name, pe.pet_image, po.photo, po.comment, po.good, po.date_created, COUNT(*) AS good 
...
// 同じ会員の他のペット情報を取得
$sql_post = 'SELECT id, pet_name, pet_image FROM pets WHERE member_id=? AND id NOT IN (?) ORDER BY id DESC';
$get_same_pets = $db->prepare($sql_post);
$get_same_pets->execute(array($clicked_post['member_id'], $clicked_post['pet_id']));
$same_pets = $get_same_pets->fetchAll(PDO::FETCH_ASSOC);
//ペット以外の動物の投稿を取得
$sql =
'SELECT id, photo, comment, pet_id, date_created
FROM posts 
WHERE pet_id=0 AND member_id=? AND NOT id=?
ORDER BY id DESC';
$get_other_posts = $db->prepare($sql);
$get_other_posts->execute(array(
    $clicked_post['member_id'],
    $_REQUEST['id']
));
...
//投稿削除
if ($clicked_post['member_id'] === $_SESSION['login']['member_id'] && $_POST['delete'] === 'ok') {
...
?>
    <section class="contents">
        <?php if ($clicked_post['pet_id'] != 0 && !is_null($clicked_post['name'])): ?>
        <t2 class="post_t2">
                <img class="icon" <?php print('src="/pets/images/' . $clicked_post['pet_image'] . '"'); ?>>
            <?php print(htmlspecialchars($clicked_post['pet_name'])); ?>
        </t2>
        <?php endif; ?>
        <?php if (!is_null($clicked_post['name'])): ?>
        <p>
            投稿者：<?php print(htmlspecialchars($clicked_post['name'])); ?>
        </p>
        <?php endif; ?>
        <p>
        ...
        <?php if ($clicked_post['member_id'] === $_SESSION['login']['member_id']): ?>
        <div id="post_delete">
            この投稿を削除する
        </div>
        ...
        <?php if (!empty($_SESSION['login']) && $clicked_post['member_id'] !== $_SESSION['login']['member_id'] && $clicked_post['pet_id'] != 0):
...
<!-- 同じ会員の他のペットの投稿を表示 -->
        <?php
            if ((!empty($same_pets) || count($other_posts) !==0) && (!is_null($clicked_post['name']))):
        ?>
        <t2 class="post_t2">
            <?php print(htmlspecialchars($clicked_post['name'])); ?>さんの他のペットの投稿を見る
        </t2><br>
        <?php 
        foreach($same_pets as $pet): ?>
            <?php //if ($pet['id'] !== 0): ?>
            <t3 class="post_t3">
            ...
            <?php if (!empty($_SESSION['login']) && $clicked_post['member_id'] !== $_SESSION['login']['member_id']):
